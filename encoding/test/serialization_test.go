...
func TestMarshal(t *testing.T) {
	...
	enc, err := encoding.Marshal(&h, binary.LittleEndian)
	if err != nil {
		t.Error(err)
	}
...
func TestUnmarshal(t *testing.T) {
	...
	err = encoding.Unmarshal(reader, &nh, binary.LittleEndian)
	if err != nil {
		t.Error(err)
	}
	t.Log(nh)
...
func TestUnmarshalInitSlice(t *testing.T) {
	array := []byte{41, 42, 43}
	slice := make([]byte, 3)
	data, err := encoding.Marshal(&array, binary.LittleEndian)
	if err != nil {
		t.Error(err)
	}
	t.Logf("data: %q\n", data)
	...
	len := int64(0)
	err = encoding.Unmarshal(reader, &len, binary.LittleEndian)
	if err != nil {
		t.Error(err)
	}
	err = encoding.UnmarshaInitSlice(reader, &slice, binary.LittleEndian)
	if err != nil {
		t.Error(err)
	}
