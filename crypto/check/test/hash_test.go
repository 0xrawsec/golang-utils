...
import (
	...
	"github.com/0xrawsec/golang-utils/crypto/check"
)
...
func TestHashVerification(t *testing.T) {
	switch {
	case !check.IsValidHash(md5):
		t.Log("MD5 not valid")
		t.Fail()
	case !check.IsValidHash(sha1):
		t.Log("SHA1 not valid")
		t.Fail()
	case !check.IsValidHash(sha256):
		t.Log("SHA256 not valid")
		t.Fail()
	case !check.IsValidHash(sha512):
